...
E.fae_P.rett_coxph <- coxph(Surv(day, status) ~ rep+absorbanceLog+primary+rep:absorbanceLog+rep:primary+primary:absorbanceLog, data = E.fae_P.rett)
...
#Generate a Cox proportional Hazards model which assesses the contribution of infectious dose while controlling for replicate
P.snee_extremes_coxph <- coxph(Surv(day, status) ~ rep+absorbanceLog+primary+rep:absorbanceLog+rep:primary+primary:absorbanceLog, data = P.snee_extremes)
...
this is being really difficult because now I am changing the primary not the secondary dose
I can't use absorbanceLog if I include the ones without chronic infections

so here it is without the PBS-P.rett condition included

GIVING UP HERE BECAUSE I AM HUNGRY AND I NEED TO TALK TO CHAMBERS ABOUT HOW TO APPROACH THIS
```{r}
#Varied primary dose
...
#Generate a Cox proportional Hazards model which assesses the contribution of infectious dose while controlling for replicate
P.rett_varied_primary_coxph <- coxph(Surv(day, status) ~ rep + primary_dose + rep:primary_dose, data = P.rett_varied_primary)
summary(P.rett_varied_primary_coxph)
anova(P.rett_varied_primary_coxph)

#separate by dose to see if the significance if dependent on dose
P.rett_varied_primary_0.001 = subset(P.rett_varied_primary, primary_dose == 0.001)
P.rett_varied_primary_0.001 <- droplevels(P.rett_varied_primary_0.001)
P.rett_varied_primary_0.001_coxph <- coxph(Surv(day, status) ~ rep + primary_dose + secondary + rep:primary_dose, data = P.rett_varied_primary_0.001)
#summary(P.rett_varied_primary_0.001_coxph)
anova(P.rett_varied_primary_0.001_coxph)

table(P.rett_varied_primary$secondary)
...
P.rett_varied_primary_0.01_coxph <- coxph(Surv(day, status) ~ rep + primary_dose + secondary + rep:primary_dose + rep:secondary, data = P.rett_varied_primary_0.01)
#summary(P.rett_varied_primary_0.01_coxph)
anova(P.rett_varied_primary_0.01_coxph)
```
and here it is using absorbance instead of absorbanceLog
```{r}
#P.rett_varied_primary$absorbanceLog <- log10(P.rett_varied_primary$primary_dose)
P.rett_varied_primary <- droplevels(P.rett_varied_primary)

#Generate a Cox proportional Hazards model which assesses the contribution of infectious dose while controlling for replicate
P.rett_varied_primary_coxph <- coxph(Surv(day, status) ~ rep + primary_dose + secondary + rep:primary_dose + rep:secondary + primary_dose:secondary, data = P.rett_varied_primary)
summary(P.rett_varied_primary_coxph)
anova(P.rett_varied_primary_coxph)

table(P.rett_varied_primary$primary)

#separate by dose to see if the significance if dependent on dose
P.rett_varied_primary_0.001 = subset(P.rett_varied_primary, primary_dose == 0 | primary_dose == 0.001)
P.rett_varied_primary_0.001 <- droplevels(P.rett_varied_primary_0.001)
P.rett_varied_primary_0.001_coxph <- coxph(Surv(day, status) ~ rep + primary + secondary + rep:primary + rep:secondary, data = P.rett_varied_primary_0.001)
summary(P.rett_varied_primary_0.001_coxph)
anova(P.rett_varied_primary_0.001_coxph)

P.rett_varied_primary_0.01 = subset(P.rett_varied_primary, primary_dose == 0.01 | primary_dose == 0)
P.rett_varied_primary_0.01 <- droplevels(P.rett_varied_primary_0.01)
P.rett_varied_primary_0.01_coxph <- coxph(Surv(day, status) ~ primary_dose + secondary, data = P.rett_varied_primary_0.01)
#summary(P.rett_varied_primary_0.01_coxph)
anova(P.rett_varied_primary_0.01_coxph)
...
P.rett_varied_primary_0.1_coxph <- coxph(Surv(day, status) ~ primary_dose + secondary, data = P.rett_varied_primary_0.1)
...
P.rett_varied_primary_1_coxph <- coxph(Surv(day, status) ~ rep + primary + secondary + rep:primary + rep:secondary, data = P.rett_varied_primary_1)
